...
{
    public partial class ChallPriceService : IChallPriceService
    {
        private readonly IPriceFeeDataBaseContext _context;
        private readonly IPowerCalculationService _powerCalculationService;
        public ChallPriceService(IPriceFeeDataBaseContext Context, IPowerCalculationService powerCalculationService)
        {
            _context = Context;
        ...
        private ResultChallCostDto falseResult = new ResultChallCostDto
        {
            ProductCost = -1,
            edgeCost = -1,
            edgeWorkerCost = -1,
        ...
        public ResultDto<ResultChallCostDto, CalculationResult> Execute(RequestChallCostDto request1, RequestNfpInfoDto request2, string QualityFactor)
        {


            try
            {
                var EdgeContent = _context.EdgeProperties.FirstOrDefault(m => m.EdgeSize == request1.EdgeSize && m.EdgeColor == request1.EdgeColor && m.EdgeTitle == request1.Title && m.QualityFactor == QualityFactor);
                if (EdgeContent != null) {
                ...
                {
                    return new ResultDto<ResultChallCostDto, CalculationResult>()
                    {
                        Data = falseResult,
                        IsSuccess = false,
                ...
                if (request1.PowerCheckpoint == true)
                {
                    if (request1.PowerCalculationType == ConstantMaterialName.selfchoice | request1.PowerCalculationType == null)
                    {
                        ...
                        Console.WriteLine(QualityFactor);
        ...
        public ResultDto<AllQfChallResultDto> AllQfCalculation(RequestChallCostDto request1, RequestNfpInfoDto request2)
        {
            ...
            {
                return new ResultDto<AllQfChallResultDto>
                {

                    Data = new AllQfChallResultDto(),
                    IsSuccess = false,
