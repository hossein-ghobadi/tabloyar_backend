...
{
    public class SwediMaxPriceService : ISwediMaxPriceService
    {

        private readonly IPriceFeeDataBaseContext _context;
        private readonly IPowerCalculationService _powerCalculationService;
        public SwediMaxPriceService(IPriceFeeDataBaseContext Context, IPowerCalculationService powerCalculationService)
        {
            _context = Context;
        ...
        private ResultSwediMaxPriceDto falseResult = new ResultSwediMaxPriceDto
        {
            ProductCost = -1,
            edgeCost = -1,
            edgeWorkerCost = -1,
        ...
        public ResultDto<ResultSwediMaxPriceDto, CalculationResult> Execute(RequestSwediMaxPriceDto request1, RequestSwediMaxNfpInfoDto request2, string QualityFactor)
        {
            try
            {


                var EdgeContent = _context.EdgeProperties.FirstOrDefault(m => m.EdgeSize == request1.EdgeSize && m.EdgeTitle == request1.Title && m.QualityFactor == QualityFactor);//&& m.EdgeColor == request1.EdgeColor && m.EdgeSecondColor == request1.EdgeSecondColor
                if (EdgeContent != null)
                {
                    var EdgeFee = EdgeContent.EdgeFee;
                    EdgeCost = request2.LRealPvc * EdgeFee;
                }
                else
                {
                    return new ResultDto<ResultSwediMaxPriceDto, CalculationResult>()
                    {
                        Data = falseResult,
                        IsSuccess = false,
                ...
                //    else
                //    {
                //        return new ResultDto<ResultSwediMaxPriceDto>()
                //        {
                //            Data = falseResult,
                //            IsSuccess = false,
                ...
                return new ResultDto<ResultSwediMaxPriceDto, CalculationResult>
                {
                    Data = new ResultSwediMaxPriceDto
                    {
        ...
        public ResultDto<AllQfSwediMaxResultDto> AllQfCalculation(RequestSwediMaxPriceDto request1, RequestSwediMaxNfpInfoDto request2)
        {
            ...
            {
                return new ResultDto<AllQfSwediMaxResultDto>
                {

                    Data = new AllQfSwediMaxResultDto(),
                    IsSuccess = false,
