...
{
    public class SwediPriceService : ISwediPriceService
    {
        private readonly IPriceFeeDataBaseContext _context;
        private readonly IPowerCalculationService _powerCalculationService;
        public SwediPriceService(IPriceFeeDataBaseContext Context, IPowerCalculationService powerCalculationService)
        {
            _context = Context;
        ...
        private ResultSwediPriceDto falseResult = new ResultSwediPriceDto
        {
            ProductCost = -1,
            edgeCost = -1,
            edgeWorkerCost = -1,
        ...
        public ResultDto<ResultSwediPriceDto, CalculationResult> Execute(RequestSwediPriceDto request1, RequestSwediNfpInfoDto request2, string QualityFactor)
        {
            try
            {
                ...
                var EdgeContent = _context.EdgeProperties.FirstOrDefault(m => m.EdgeSize == request1.EdgeSize  && m.EdgeTitle == request1.Title && m.QualityFactor == QualityFactor);//&& m.EdgeColor == request1.EdgeColor
                if (EdgeContent != null)
                {
                ...
                {
                    return new ResultDto<ResultSwediPriceDto, CalculationResult>()
                    {
                        Data = falseResult,
                        IsSuccess = false,
                ...
                //    else
                //    {
                //        return new ResultDto<ResultSwediPriceDto>()
                //        {
                //            Data = falseResult,
                //            IsSuccess = false,
                ...
                if (request1.BSmdModel != null && request1.BSmdCheckpoint == true)
                {
                    ...
                    else
                    {
                        return new ResultDto<ResultSwediPriceDto, CalculationResult>()
                        {
                            Data = falseResult,
                            IsSuccess = false,
                            Message = "اس ام دی بکلایت وجود ندارد"
                        };
                    }
                }
        ...
        public ResultDto<AllQfSwediResultDto> AllQfCalculation(RequestSwediPriceDto request1, RequestSwediNfpInfoDto request2)
        {
            ...
            {
                return new ResultDto<AllQfSwediResultDto>
                {

                    Data = new AllQfSwediResultDto(),
                    IsSuccess = false,
